...
export function DashboardWelcomeBanner() {
  ...
  const dateCardBg = isDark ? "bg-white/10 border-white/10 text-white" : "bg-gray-50 border-gray-300 text-gray-800 shadow-sm";
    ...
    >
      ...
      <div className="relative z-10 flex flex-col sm:flex-row sm:items-center gap-6 justify-between">
        <div className="flex items-center gap-5">
          <div className={`w-16 h-16 rounded-2xl flex items-center justify-center flex-shrink-0 backdrop-blur-md shadow-inner border ${isDark ? "bg-white/10 border-white/20" : "bg-white border-gray-200"}`}>
            <Shield className="w-8 h-8 text-red-500" />
          </div>
          <div>
            <p className="text-red-500 text-sm mb-1 font-medium tracking-wide uppercase">DASHBOARD IKHTISAR</p>
            <h1 className={`text-2xl sm:text-3xl font-bold tracking-tight drop-shadow-sm ${textPrimary}`}>Welcome Back, {adminName}</h1>
          </div>
        </div>
        <div className={`mt-4 sm:mt-0 px-4 py-2 rounded-xl backdrop-blur-md border inline-flex items-center justify-center sm:text-right w-fit sm:w-auto self-start sm:self-auto shadow-inner ${dateCardBg}`}>
          <p className="text-sm font-medium tabular-nums tracking-wide">
            {formatDate(time)} <span className="text-gray-400 mx-2">|</span> <span className="text-red-500">{formatTime(time)}</span>
          </p>
        </div>
      </div>
    </motion.div>
