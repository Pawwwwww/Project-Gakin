...
function FlippableCard({ data }: { data: typeof INFO_DATA[0] }) {
  ...
  return (
    ...
    >
      ...
      >
        {/* FRONT SIDE */}
        <div className={`absolute inset-0 backface-hidden flex flex-col items-center justify-center p-6 rounded-2xl border ${cardBg} shadow-sm backdrop-blur-xl group-hover:border-red-400/30 transition-colors`}>
          ...
          <div className={`absolute bottom-4 right-4 w-6 h-6 rounded-full flex items-center justify-center ${isDark ? "bg-white/10 text-gray-400" : "bg-gray-100 text-gray-400"}`}>
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="mt-8">
      <div className="mb-6">
        <h3 className={`text-xl font-bold flex items-center gap-2 ${textPrimary}`}>
          <Info className="w-5 h-5 text-red-500" /> Informasi Kuesioner
        </h3>
